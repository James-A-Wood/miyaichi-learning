...
define(
    ...
    function($, tools) {
        ...
        return function(inputs) {
            ...
            const isMobile = tools.isMobile() && window.innerWidth < 500;
            ...
            isMobile && $("#floating-score-number-mistakes").css({
                float: "right"
            });

            // marking the distance from the top
            const originalDistanceFromTop = $(".floating-score-box").offset().top;

            // setting the previous state
            let previousState = floatingStyling;

            // wiringup scroll monitoring
            $(window).scroll(function() {

                const windowTop = $(window).scrollTop() + parseInt(defaults.topBuffer);
                const currentState = (windowTop > originalDistanceFromTop) ? stickyStyling : floatingStyling;

                // only switching styles if there's been a change
                if (currentState !== previousState) {
                    $("#floating-score-box").css(currentState);
                    previousState = currentState; // marking the previous state
                }
            });
